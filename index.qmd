...
* The [`crew.cluster`](https://wlandau.github.io/crew.cluster/) package has launcher plugins for traditional computing clusters such as SLURM, Sun Grid Engine, LSF, and PBS/TORQUE.
* Users can write custom plugins for other platforms, such as cloud technologies like AWS Batch, Google Cloud Run, and Kubernetes.
...
```{r, eval = FALSE, echo = TRUE, attr.source = '.numberLines startFrom="49"'}
  ...
  observe({
    ...
    if (!is.null(result)) reactive_result(result[[1L]])
    reactive_status(status_message(number_tasks = length(controller$queue)))
    reactive_poll(!controller$empty())
  })
}
...
* Plenty of room for AWS Batch, AWS Lambda, Google Cloud Run, Kubernetes, and beyond.
